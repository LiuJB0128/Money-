...
export const EmojiSelect = defineComponent({
  ...
  setup: (props, context) => {
    ...
    const table: [string, string[]][] = [
      ...
      ['出行', ['transport-ground', 'transport-water', 'transport-air', 'hotel',
        'place-building', 'place-geographic'
      ]],
    ...
    const emojis = computed(() => {
      return table[refSelected.value][1].map(category =>
        emojiList.find(item => item[0] === category)?.[1]
          .map(item => <li class={ item === props.modelValue ? styles.selectedEmoji : '' }
            onClick={ () => onClickEmoji(item) }>{ item }</li>)
      )
    })
    return () => (
      <div class={ styles.emojiList }>
        <nav>
          {table.map((item, index) =>
            <span class={ index === refSelected.value ? styles.selected : '' }
